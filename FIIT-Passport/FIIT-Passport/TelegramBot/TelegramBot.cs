...
public class TelegramBot(TelegramDbContext repo, ITelegramBotClient botClient)
{
    ...
    private async Task CheckConfirmUser(string telegramTag, ITempDataDictionary tempData)
    {
    ...
    private async Task<bool> CheckStartUser(string telegramTag, ITempDataDictionary tempData)
    {
        ...
        if (await _authenticationTriggersStart[telegramTag].WaitAsync(TimeSpan.FromSeconds(5))) return true;
        tempData["error"] = "Превышение времени ожидания";
        _authenticationTriggersStart.TryRemove(telegramTag, out _);
        return false;
    }
    
    public async Task AuthenticationUser(string telegramTag, ITempDataDictionary tempData)
    {
        if (await repo.CheckUser(telegramTag))
        {
            await CheckConfirmUser(telegramTag, tempData);
            return;
        }
        if (!await CheckStartUser(telegramTag, tempData))
            return;
        await CheckConfirmUser(telegramTag, tempData);
        tempData["success"] = "Ваша личность подтверждена";
    }
